...
List<Categories> categoriesListFromJson(String val) => List<Categories>.from(
    json.decode(val)['categories'].map((data) => Categories.fromJson(data)));
...
class Categories {
  ...
  factory Categories.fromJson(Map<dynamic, dynamic> data) => Categories(
        id: data['id'],
        ...
        product_amount: List<int>.from(
            data['products'].map((data) => data['product_quantity'] ?? 0)),
        Categories_id: List<int>.from(
            data['products'].map((data) => data['Categories_id'] ?? 0)),
